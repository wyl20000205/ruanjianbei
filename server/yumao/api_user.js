...
route.post('/set_user_info', async (req, res) => {
    ...
    if (filed == 'pass') {
        if (user_data.pass == info.old_pass) {
            msg.msg = 'success!'
            msg.code = 200
            await tool.set_table_filed('t_user', filed, info.new_pass, uid);
        }
    } else if (filed == 'name') {
        msg.msg = 'success!'
        msg.code = 200
        await tool.set_table_filed('t_user', filed, info.name, uid);
    } else if (filed == 'user') {
        let temp = await tool.check_exist('user', 'user', info.user)
        if (!temp || temp.id == uid) {
            msg.msg = 'success!'
            msg.code = 200
            await tool.set_table_filed('t_user', filed, info.user, uid);
        }
    } else if (filed == 'mail') {
        ...
        if (!temp || temp.id == uid) {
            msg.msg = 'success!'
            msg.code = 200
            await tool.set_table_filed('t_user', filed, info.mail, uid);
        }
    }
    res.send(msg)
})
