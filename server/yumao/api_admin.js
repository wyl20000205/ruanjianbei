...
route.post('/set_swiper', tool.upload, async (req, res) => {
    ...
    config.db.execute('delete from t_swiper')
...
route.post('/set_news', async (req, res) => {
    ...
    config.db.execute('update t_news set title = ? , link = ? where id = ?', [title, link, id])
    res.send('200')
})
//设置其他信息
route.post('/set_other', async (req, res) => {
    let { content, id } = req.body
    config.db.execute('update t_detail set content = ? where id = ?', [content, id])
    res.send('200')
})
//获取员工信息
...
route.post('/save_food', tool.upload, async (req, res) => {
    ...
    db.execute('update t_food set name=?,detail=?,introduce=?,localtion=?,price=?,img=? where id = ?', [name, detail, introduce, localtion, price, new_img, id])
    res.send('200')
})
//删除食物
route.post('/delete_food', async (req, res) => {
    let { name, detail, introduce, localtion, price, img, id } = req.body
    config.db.execute('delete from t_food where id =  ?', [id])
    if (img != 'default.jpg') fs.removeSync(imgUrl + img)
    res.send('200')
})
//删除评价
route.post('/estimate_delete', async (req, res) => {
    config.db.execute('update t_trade set estimate=-2 where id = ?', [req.body.id])
    res.send({ message: 'success', code: 200 })
})
//编辑员工
route.post('/save_staff', async (req, res) => {
    let { name, sort, tel, localtion, id } = req.body
    config.db.execute('update t_staff set name=?,sort=?,tel=?,localtion=? where id = ?', [name, sort, tel, localtion, id])
    res.send('200')
})
//删除员工
route.post('/delete_staff', async (req, res) => {
    let { id } = req.body
    config.db.execute('delete from t_staff where id =  ?', [id])
    res.send('200')
})
//编辑用户
route.post('/save_user', async (req, res) => {
    ...
    if (temp_01 && temp_01.id != id)
        ...
        config.db.execute('update t_user set user=?,mail=?,pass=?,price=?,name=? where id = ?', [user, mail, pass, price, name, id])
...
route.post('/delete_user', async (req, res) => {
    let { id } = req.body
    config.db.execute('delete from t_user where id =  ?', [id])
    res.send('200')
})
//添加员工
