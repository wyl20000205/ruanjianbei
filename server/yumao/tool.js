...
export default {
    ...
    async user_check(info) {
        return new Promise(async (solve, reject) => {
            ...
            if (!data_01 && !data_02)
                db.execute(`insert into t_user(user,pass,mail,name) value(?,?,?,'用户_${parseInt(Math.random() * 10000000)}')`, [user, pass, mail])
            solve(msg)
        })
    },
    //用户账号找回
    ...
    async set_account(info) {
        return new Promise(async (solve, reject) => {
            ...
            db.execute('update t_admin set user=?,mail=?,pass=? where id=?', [user, mail, new_pass, admin_id], (err, result) => {
                if (err) solve(0)
                else solve(1)
            })
        })
    ...
    async search_user(text) {
        return new Promise(async (solve, reject) => {
            db.execute(`select * from t_user where user like '%${text}%' or pass like '%${text}%' or mail like '%${text}%' or id like '%${text}%' or name like '%${text}%' order by id desc`, (err, result) => {
                solve(result)
            })
        })
    },
    ...
    async add_food(data) {
        ...
        console.log(data);
        return new Promise(async (solve, reject) => {
            db.execute("insert into t_food(name,detail,price,localtion,introduce,img) value(?,?,?,?,?,?)", [name, detail, price, localtion, introduce, img], (err, result) => {
                solve(result)
            })
        })
    },
    ...
    async get_business_trade(name, index) {
        if (index == 1) {
            return new Promise((solve, reject) => {
                db.execute("select * from t_trade where  food_localtion = ? and status = 1 order by id desc", [name], (e, r) => {
                    if (r) {
                        r = this.filter_time(r)
                        solve(r)
                    }
